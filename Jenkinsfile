	...
	try {
		...
		timestamps() {
			stage('Checkout') {
				...
				minimums = getCoverageMinimums("Studio/studio3-php/${targetBranch}", isPR)
			}
...
"""
				timeout(10) {
					sh 'ant -propertyfile override.properties -f builders/com.aptana.php.build/build.xml build'
				}
...
php.p2.repo=file://${env.WORKSPACE}/dist/
"""
						timeout(10) {
							sh 'ant -propertyfile test-override.properties -f builders/com.aptana.php.tests.build/build.xml build test'
						} // end timeout
					} // end mask passwords
				} // end xvnc
