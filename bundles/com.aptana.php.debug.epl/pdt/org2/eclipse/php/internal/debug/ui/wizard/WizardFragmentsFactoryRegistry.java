...
public class WizardFragmentsFactoryRegistry
{
	...
	/**
	 * Returns an array on newly initialized WizardFragments that complies to the given server type id. The returned
	 * fragments array contains a union of the server specific fragments and the global fragments that can be defined by
	 * adding a fragments extention with a visibility of 'Always'.
	 * 
	 * @param serverType
	...
	private void sortFragmentsByPlace(ArrayList fragments)
	{
		...
		for (int i = 0; i < nonRootFragments.size(); i++)
		{
			...
			if (!moved)
			{
				...
				// PHPUiPlugin.log(new Status(IStatus.WARNING, PHPUiPlugin.ID, 0, "Invalid 'placeAfter' id (" +
				// invalidFactory.getPlaceAfter() + ')', null)); //$NON-NLS-1$
			}
		}
		...
		public ICompositeFragmentFactory createFragmentFactory()
		{
			SafeRunner
					.run(new SafeRunnable(
							"Error creation extension for extension-point org2.eclipse.php.server.ui.wizardAndCompositeFragments") { //$NON-NLS-1$
