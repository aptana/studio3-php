...
/**
 * The PHP launch delegate proxy is designed to supply flexibility in delegating launches 
 * to different types of launch configuration delegates.
 * Using the proxy model can allow a runtime determination of the right delegate to launch.
 * 
 * @author Shalom Gibly
 *
 */
public class PHPLaunchDelegateProxy implements ILaunchConfigurationDelegate2 {
	 ...
	 * define the plug-in as Eclipse-RegisterBuddy: org2.eclipse.php.debug.core
	 * 
	...
	protected ILaunchConfigurationDelegate2 getConfigurationDelegate(ILaunchConfiguration configuration) throws CoreException {
		if (launchConfigurationDelegate == null) {
			try {
				...
				throw new CoreException(new Status(IStatus.ERROR, PHPDebugEPLPlugin.PLUGIN_ID, 0, "Launch configuration delegate loading error.", t)); //$NON-NLS-1$
			}
		}
