...
php.formatter.spaces.before.conditional=0
...
php.formatter.spaces.before.relational=0
...
php.formatter.spaces.after.conditional=0
php.formatter.spaces.after.dot=1
php.formatter.spaces.after.relational=0
...
class AA extends PDO {
	...
	static abstract   function foo() {
		...
		$blah = 18;
		$someFloat  =   3.14  ;
		    if ( $something  != "" && $somethingElse  &=   "" || $blah > 2) 
		    {
			preg_replace(   $pattern, $replacement  ,    $subject);
			str_ireplace( $search  , $replace,  $subject);
			$aaa =  DEFAULT_INCLUDE_PATH;
			++  $blah;
		}
	}
}
...
class AA extends PDO {
	const DDA = s;
	const NO_COLOR = true;
	public static abstract function foo() {
		$a = new PDO;
		$blah = 18;
		$someFloat = 3.14;
		if ($something!="" && $somethingElse &= "" || $blah>2) {
