...
import org2.eclipse.php.internal.core.ast.util.Signature;

/**
 * Stores all rewrite events, descriptions of events and knows which nodes are
 * copy or move sources or tracked.
 */
...
public final class RewriteEventStore {
	...
	public static class CopySourceInfo implements Comparable {
		public final PropertyLocation location; // can be null, only used to
												// mark as removed on move
	...
	private static class NodeRangeInfo implements Comparable {
		...
		public final CopySourceInfo copyInfo; // containing the internal
												// placeholder and the 'isMove'
												// flag
	...
	private RewriteEvent[] processListWithRanges(List rangeInfos,
		...
		for (int k = 0; k < childEvents.length; k++) {
			...
			while (nextInfo != null && node == nextInfo.getStartNode()) { // is
																			// this
																			// child
																			// the
																			// beginning
