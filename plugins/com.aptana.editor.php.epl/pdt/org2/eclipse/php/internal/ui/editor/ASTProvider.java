 ...
 *     Zend Technologies
 *******************************************************************************/
package org2.eclipse.php.internal.ui.editor;
...
public final class ASTProvider {
	...
	private class ActivationListener implements IPartListener2, IWindowListener {
		...
		private boolean isPHPEditor(IWorkbenchPartReference ref) {
			...
			// [Aptana Mod - Use ISourceModuleProviderEditor]
			IWorkbenchPart part = ref.getPart(false);
			if (part instanceof ISourceModuleProviderEditor)
			{
				ISourceModuleProviderEditor editor = (ISourceModuleProviderEditor) part;
				return editor.getEditorID().equals(id);
			}
			return false;
		}
	}

	public static final PHPVersion SHARED_AST_LEVEL = PHPVersion.getLatest();
	...
	public static ASTProvider getASTProvider() {
		return PHPEplPlugin.getDefault().getASTProvider();
	}
	...
	private void activeJavaEditorChanged(IWorkbenchPart editor) {
		...
		if (editor instanceof ISourceModuleProviderEditor)
			phpElement = ((ISourceModuleProviderEditor) editor).getSourceModule();
		...
		if (DEBUG)
			System.out
					.println(getThreadName()
							+ " - " + DEBUG_PREFIX + "active editor is: " + toString(phpElement)); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private synchronized void disposeAST() {
		...
		if (DEBUG)
			System.out
					.println(getThreadName()
							+ " - " + DEBUG_PREFIX + "disposing AST: " + toString(fAST) + " for: " + toString(fActivePHPElement)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	private synchronized void cache(Program ast, ISourceModule javaElement) {
		...
		if (fActivePHPElement != null
				&& !fActivePHPElement.equals(javaElement)) {
