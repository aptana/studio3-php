 ...
 */
package org.eclipse.php.internal.debug.core.interpreter.preferences;
...
public final class DependenciesManager
{
	...
	public static void setExtensions(List<PHPExtension>extensions){
		...
		PHPDebugEPLPlugin.getDefault().getPreferenceStore().setValue("php-extensions",bld.toString()); //$NON-NLS-1$		
	}


	private static QualifiedName buildPathPropertyName()
	{
		return new QualifiedName(PHPDebugEPLPlugin.PLUGIN_ID, "phpbuildpath"); //$NON-NLS-1$
	}
	
	private static QualifiedName libsPropertyName()
	{
		return new QualifiedName(PHPDebugEPLPlugin.PLUGIN_ID, "libs"); //$NON-NLS-1$
	}
	...
	public static List<PHPExtension> getExtensions(){
		String string = PHPDebugEPLPlugin.getDefault().getPreferenceStore().getString("php-extensions"); //$NON-NLS-1$
