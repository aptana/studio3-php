...
public class PHPsComboBlock implements ISelectionProvider {
	...
	public PHPsComboBlock(boolean titleGrouped) {
		...
		fDefaultDescriptor = new PHPexeDescriptor() {
			public String getDescription() {
				final PHPexeItem def = exes.getDefaultItem(PHPDebugEPLPlugin.getCurrentDebuggerId());
				if (def != null)
	...
	protected void fillWithWorkspacePHPexes() {
		...
		final PHPexeItem[] types = exes.getItems(PHPDebugEPLPlugin.getCurrentDebuggerId());
		if (types != null) {
					...
					fExecutablesCombo.getDisplay().syncExec(new Runnable()
					{
						public void run()
						{
							try
							{
								...
								{
									fExecutablesCombo.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_YELLOW));
								}
								else
								{
									fExecutablesCombo.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_LIST_BACKGROUND));
								}
							}
