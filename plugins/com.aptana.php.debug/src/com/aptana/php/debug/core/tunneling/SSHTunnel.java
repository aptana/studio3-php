...
/**
 * An SSH tunnel (port forwarding). This class creates a tunnel between a port on a remote host and a port on the local
 * host.
 * 
 * @author Shalom Gibly
 */
public class SSHTunnel
{
	 ...
	 *             In case one of the variables is null or off limits (null passwords are not verified).
	 */
	...
	/**
	 * Create the tunnel connection. If the tunnel is already connected, nothing happens. This call will potentially
	 * change the password value of this class in case the user is prompted to enter a valid one. There are several
	...
	public IStatus connect()
	{
		...
		while (true)
		{
			try
			{
				session = SSHTunnelSession.getSession(userName, password, remoteHost, SSH_DEFAULT_PORT, null)
						.getSession();
				// This might throw exception because of binary compatibility issues.
				...
				catch (InvocationTargetException e)
				{
					...
					if (e.getCause() != null && e.getCause().getMessage() != null
							&& e.getCause().getMessage().toLowerCase().indexOf("already registered") == -1) //$NON-NLS-1$
					{
						ex = e;
					}
				}
		...
		if (status.isOK() && newPassword != null)
		{
			...
			// the caller.
			status = new Status(IStatus.INFO, PHPDebugPlugin.PLUGIN_ID, CONNECTION_PASSWORD_CHANGED_CODE, newPassword,
					null);
		}
	...
	/**
	 * Returns the password. The returned password might differ from the original given one in case that the user was
	 * requested to change it while connecting.
	 * 
	 * @return The password.
	 */
	...
	/**
	 * Check for SSHTunnel equality. Note that we do no compare passwords, as the password itself can be modified during
	 * connections.
	 * 
