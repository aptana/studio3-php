...
/**
 * A generic debugger configuration that should be implemented by any debugger that is 
 * attached to the phpDebuggers extension point.
 * The debugger configuration supplies specific debugger attributes as well as the option
 * to obtain other attributes.
 * Also, the debugger configuration can contribute to the UI by creating a Composite widget for setting
 * its attributes.
 * 
 * The IDebuggerConfiguration is connected to the PHP debug plug-in in a way that any attribute that exists
 * in the preferences is returned when requested.
 * Attributes that do not exist in the preferences are held in this runtime instance of the debugger configuration
...
public interface IDebuggerConfiguration {
	...
	/**
	 * Attribute key for the debugger's delegate for executing PHP scripts using a local PHP executable.
	 */
