 ...
 *******************************************************************************/
package org2.eclipse.php.internal.debug.core.xdebug.dbgp.session;
		...
		try {
			...
			if (response != null) {
				...
				if (DBGpResponse.INIT == parsedResponse.getType()) {
					...
					if (ideKey.startsWith(DBGpSessionHandler.IDEKEY_PREFIX + '_'))
					{
						sessionId = ideKey.substring(DBGpSessionHandler.IDEKEY_PREFIX.length() + 1);
						ideKey = DBGpSessionHandler.IDEKEY_PREFIX;
					}
					else
					{
